...
Now SOAP is done. Save the results again in the results folder!
...
When completed there is a file of interest here: `coverage.hist` as generated by ABYSS. This is 
the K-mer histogram. 
...
like "coverage" hit the **<kbd>Enter</kbd>** key (there will be no prompt after 
...
The `short.hist.txt` is a tab-delimited file, so we can open it using Excel 
or another spreadsheet program and create
...
Remember that the `^` is a special character that tells `grep` to only match the pattern 
...
But this is not in the `o<username> file. 
