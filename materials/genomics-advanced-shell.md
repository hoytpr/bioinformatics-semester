...
return the line immediately before each match and the two lines immediately after each match (a total of 4 lines).

We can use the `-B` argument for `grep` to return a specific number of lines **B**efore
each match. The `-A` argument returns a specific number of lines **A**fter each matching line. Here we want the line *before* and the two lines *after* each 
...
To prevent extra separators from being introduced you can use the `--no-group-separator` option. 

```
$ grep -B1 -A2 NNNNNNNNNN SRR098026.fastq --no-group-separator
```

> #### Preview, then redirect output
> 
> `grep` allowed us to identify sequences in our FASTQ files that match a particular pattern. 
> We captured that output using "redirection" `>`. we used what is printed to the terminal 
> screen before printing the output to a file and it allowed us to see problems. There are 
...
When we send multiple files to the `grep` command, the output lines are prefaced with the file name. This is to help us find the actual lines later. If we want to stop this from happening (in order to have a perfectly formatted `fastq` file as the output), we can use the `-h` flag. 

```
grep -h -A1 -B1 AAGTT *.fastq --no-group-separator
```
...
~~~
$ grep -h -B1 -A2 NNNNNNNNNN SRR098026.fastq --no-group-separator > bad_reads.txt
~~~
...
>>
>> ~~~
>> $ grep NNN SRR098026.fastq --no-group-separator > bad_reads.txt
>> $ wc -l bad_reads.txt
>> 249
>> ~~~
>>
...
~~~
$ grep -B1 -A2 NNNNNNNNNN SRR098026.fastq --no-group-separator > bad_reads.txt
$ wc -l bad_reads.txt
  536 bad_reads.txt
$ grep -B1 -A2 NNNNNNNNNN SRR097977.fastq --no-group-separator > bad_reads.txt
$ wc -l bad_reads.txt
  0 bad_reads.txt
~~~
...
~~~
$ grep -B1 -A2 NNNNNNNNNN *.fastq --no-group-separator > bad_reads.txt
$ wc -l bad_reads.txt
  536 bad_reads.txt
~~~
...
> 
> ~~~
> grep -B1 -A2 NNNNNNNNNN *.fastq --no-group-separator > bad_reads.fastq
...
~~~
$ grep -B1 -A2 NNNNNNNNNN SRR098026.fastq --no-group-separator | less
~~~
